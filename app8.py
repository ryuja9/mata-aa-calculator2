...
IMAGES_DIR = "images"  # Carpeta con cartas: 'ac.png', '10h.png', etc.
...
st.markdown("Interfaz más bonita y amigable — selecciona jugador, luego toca la carta en la pestaña del palo. Se bloquean duplicados automáticamente. ¡Disfruta!")
...
with st.expander("📖 Cómo usar (Tutorial rápido)"):
    ...
    3. Elige 'Rellenar automáticamente' o 'Elegir posición específica' (para slots exactos).
        ...
        with cols[i-1]:
            ...
            if v:
                ...
                if os.path.isfile(img_path):
                    st.markdown(f'<div class="tooltip"><img src="{img_path}" class="card-img" width="120"><span class="tooltiptext">{v.upper()}</span></div>', unsafe_allow_html=True)
                else:
                    st.write(v)
            else:
                st.write("vacío")
    ...
    with tabs[suit_chars.index(suit_char)]:
        ...
        for i, rank in enumerate(RANKS):
            ...
            with cols[i]:
                ...
                if os.path.isfile(img_path):
                    st.markdown(f'<div class="tooltip"><img src="{img_path}" class="card-img" width="80"><span class="tooltiptext">{card.upper()}</span></div>', unsafe_allow_html=True)
                else:
                    st.write(card)
                used = used_map()
